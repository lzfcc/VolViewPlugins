...
class ScalarKmeansMarkovRandomField : public FilterModuleBase {
...
public:
  ...
  typedef itk::PasteImageFilter< typename MRFFilterType::OutputImageType > PasteFilterType;


public:

  /**  Constructor */
  ScalarKmeansMarkovRandomField() 
    {
  ...
  /**  Destructor */
  virtual ~ScalarKmeansMarkovRandomField() 
    {
    }
...
double totalWeight = 0;
    ...
    typename OutputImageType::Pointer filterOutputImage;
    ...
    if (useMask)
      ...
      //
      filterOutputImage = OutputImageType::New();
    ...
    const OutputVolumePixelType fillOutsideMaskValue = 
            static_cast< OutputVolumePixelType >(numberOfClasses);
    while( !it.IsAtEnd() )
      {
      if( m_ClassificationRegion.IsInside( it.GetIndex() ) )
        {
        const OutputVolumePixelType pixelValue =
              static_cast<OutputVolumePixelType>( ( ot.Get() ) );
        *outData = pixelValue;
        ...
        {
        *outData = static_cast< OutputVolumePixelType >( fillOutsideMaskValue );
        }
      ++it;
      ++outData;
      }
