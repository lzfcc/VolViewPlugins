...
#include "vvITKSplineBoundedConnectedThreshold.txx"



template <class InputPixelType>
class SplineBoundedConnectedThresholdRunner
  {
  public:
      typedef VolView::PlugIn::SplineBoundedConnectedThreshold< 
                                            InputPixelType >   ModuleType;
  public:
    SplineBoundedConnectedThresholdRunner() {}
    void Execute( vtkVVPluginInfo *info, vtkVVProcessDataStruct *pds )
    {
      ...
      const float upper            = atof( info->GetGUIProperty(info, 4, VVP_GUI_VALUE ) );
      const float lower            = atof( info->GetGUIProperty(info, 5, VVP_GUI_VALUE ) );
      const int   replaceValue     = atoi( info->GetGUIProperty(info, 6, VVP_GUI_VALUE ) );
...
static int ProcessData(void *inf, vtkVVProcessDataStruct *pds)
{
  ...
  switch( info->InputVolumeScalarType )
    ...
    case VTK_CHAR:
      {
      SplineBoundedConnectedThresholdRunner<signed char> runner;
      runner.Execute( info, pds );
      break; 
      }
    case VTK_UNSIGNED_CHAR:
      {
      SplineBoundedConnectedThresholdRunner<unsigned char> runner;
      runner.Execute( info, pds );
      break; 
      }
    case VTK_SHORT:
      {
      SplineBoundedConnectedThresholdRunner<signed short> runner;
      runner.Execute( info, pds );
      break; 
      }
    case VTK_UNSIGNED_SHORT:
      {
      SplineBoundedConnectedThresholdRunner<unsigned short> runner;
      runner.Execute( info, pds );
      break; 
      }
    case VTK_INT:
      {
      SplineBoundedConnectedThresholdRunner<signed int> runner;
      runner.Execute( info, pds );
      break; 
      }
    case VTK_UNSIGNED_INT:
      {
      SplineBoundedConnectedThresholdRunner<unsigned int> runner;
      runner.Execute( info, pds );
      break; 
      }
    case VTK_LONG:
      {
      SplineBoundedConnectedThresholdRunner<signed long> runner;
      runner.Execute( info, pds );
      break; 
      }
    case VTK_UNSIGNED_LONG:
      {
      SplineBoundedConnectedThresholdRunner<unsigned long> runner;
      runner.Execute( info, pds );
      break; 
      }
    case VTK_FLOAT:
      {
      SplineBoundedConnectedThresholdRunner<float> runner;
      runner.Execute( info, pds );
      break; 
      }
    case VTK_DOUBLE:
      {
      SplineBoundedConnectedThresholdRunner<double> runner;
      runner.Execute( info, pds );
      break; 
      }
    }
...
static int UpdateGUI( void *inf )
{
  ...
  info->SetGUIProperty(info, 4, VVP_GUI_LABEL,"Upper Threshold");
  ...
  info->SetGUIProperty(info, 4, VVP_GUI_HELP, "Upper threshold. Only pixels with intensities lower than this threshold will be considered to be added to the region.");
  info->SetGUIProperty(info, 4, VVP_GUI_HINTS , VolView::PlugIn::FilterModuleBase::GetInputVolumeScalarRange( info ) );

  info->SetGUIProperty(info, 5, VVP_GUI_LABEL,"Lower Threshold");
  ...
  info->SetGUIProperty(info, 5, VVP_GUI_HELP, "Lower threshold. Only pixels with intensities higher than this threshold will be considered to be added to the region.");
  info->SetGUIProperty(info, 5, VVP_GUI_HINTS , VolView::PlugIn::FilterModuleBase::GetInputVolumeScalarRange( info ) );

  info->SetGUIProperty(info, 6, VVP_GUI_LABEL, "Replace Value");
  info->SetGUIProperty(info, 6, VVP_GUI_TYPE, VVP_GUI_SCALE);
  info->SetGUIProperty(info, 6, VVP_GUI_DEFAULT, "255");
  info->SetGUIProperty(info, 6, VVP_GUI_HELP, "Value to assign to the binary mask of the segmented region. The rest of the image will be set to zero.");
  info->SetGUIProperty(info, 6, VVP_GUI_HINTS , "1 255.0 1.0");
...
void VV_PLUGIN_EXPORT vvITKSplineBoundedConnectedThresholdInit(vtkVVPluginInfo *info)
{
  ...
  info->SetProperty(info, VVP_NAME, "Spline Bounded Connected Threshold (ITK)");
